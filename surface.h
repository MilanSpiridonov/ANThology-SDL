...
namespace Tmpl8 {
	...
	public:
		...
		void SetPos(float x, int y) { m_X = x; m_Y = y; }
		void SetT(Surface* t, unsigned int a_NumFrames) {m_Surface = t; m_Width =t->GetWidth(); m_Height = t->GetHeight(); }
		...
		void Move() {
			...
			{
			case 'd':
				SetPos(GetPos('X') + Spd(), GetPos('Y'));
				break;
			case 'a':
				SetPos(GetPos('X') - Spd(), GetPos('Y'));
				break;
			case 'w':
				SetPos(GetPos('X'), GetPos('Y') - Spd());
				break;
			case 's':
				SetPos(GetPos('X'), GetPos('Y') + Spd());
				break;
		...
		void SetSpd(float NewSpd) { Speed = NewSpd; }
		bool OnTriggerEnter(float x,float y,float w,float h,int Score) { //Collision system :o, could've done it way smarter but the compiler is throwing errors at me :/
			if (x + w >= GetPos('X') && GetPos('X') + GetWidth() >= x && y + h >= GetPos('Y') && GetPos('Y') + GetHeight() >= y)
			{			
				return true;			
			}
			else return false;
		...
		void Collected(float x, float y) { SetPos(x, y); }
	private:
		// Methods
		void InitializeStartData();
		// Attributes			
		char Dir;
		int m_Width = 5, m_Height = 5, m_Pitch = 5;
		float m_X, m_Y, Speed;
